...
import edu.miu.cs.cs544.examples.model.Airline;
import edu.miu.cs.cs544.examples.model.Airplane;
import edu.miu.cs.cs544.examples.model.Airport;
import edu.miu.cs.cs544.examples.model.Flight;
...
public class App {
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            System.out.println("\n-------------------Flights leaving USA with capacity > 500-------------------\n");
            // TODO update HQL
            @SuppressWarnings("unchecked")
            List<Flight> flights = session.createQuery("select f from Flight f join f.origin o join f.airplane ap where o.country = 'USA' and ap.capacity > 500").list();
        ...
        try {
            ...
            // TODO update HQL
            @SuppressWarnings("unchecked")
            List<Flight> flights = session.createQuery("select f from Flight f join f.airline al join f.airplane ap where ap.model = '747' and al.name <> 'Star Alliance'").list();
            System.out.println("Flight:  Departs:     "
                    + "                  Arrives:");
            for (Flight flight : flights) {
                System.out.printf("%-7s  %-12s %7s %8s  %-12s %7s %8s\n",
